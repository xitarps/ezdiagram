...
module EzDiagram
  ...
  def create_relation(entity, sub_entity, key)
    relation = %( "#{entity}" -> "#{sub_entity}" [arrowtail=odot, arrowhead=crow, dir=both, label="#{key.to_s}"])
    $dot_file << relation
  end

  def generate_entity(current_class)
    current_instance_methods = current_class.instance_methods(false)
                                   .map!{|variable| "#{variable.to_s.delete('@')}" + '\l\\'}.sort.join(' ')
    ...
    current_class_methods = (current_class.methods - Object.methods)
                                 .map!{|current_method| "#{current_method.to_s.delete('@')}" + '\l\\'}.sort.join(' ')
